...
export async function parsePriceHistory(productdetail:fetch.ProductDetails):Promise<parseType.PriceHistory|null>{
    try{
      return {
        ...
        pricePerQuantity: productdetail.offers[0]?.comparisonPrices[0]?.value ?? null,
        quantity: productdetail.offers[0]?.comparisonPrices[0]?.quantity ?? null,
        memberPrice:productdetail.offers[0]?.memberOnlyPrice?.value ?? null,
        originalPrice: productdetail.offers[0]?.wasPrice?.value ?? null
      }
    } catch(error){
